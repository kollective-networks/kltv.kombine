...
How the tool finds, compiles, caches and runs the scripts is described in
[How Kombine runs a script](execution.md); this document is only about the tool itself.
...
| `test` | The full tests: the `all` action of the examples runner, that is the smoke tests, the extension examples (clang build, clang docs, git, bin2cpp, bin2obj and env.win.msvc, which need the clang and llvm tools in the path) and the extras (sdl2 built from a git clone and, on Windows, msys2 packages), with one summary at the end of the whole run. The network example loads `clang.csx` from the repository URL and the extension examples load the local one, so Kombine reports once that the module is loaded from two different files in this run: expected, and the action says so before it starts. |
...
| `extensions` | The extension examples: clang (the grouped test, then the build and cleanall of the sub projects), clang docs, git, bin2cpp, bin2obj and env.win.msvc. |
| `extras` | The real world examples: sdl2 and, on Windows, msys2. |
| `all` | The three actions above in sequence. |

Every example prints a report with one aligned line per check and returns a non-zero code
when a check fails; run on its own it ends with its summary line. The runner runs every script
whatever the previous ones returned, keeps the per script summaries silent and closes the run
with one summary: the checks passed and failed per category and, for every failure, the
category, the group and the check, so a long log is searched only where it failed. It returns
1 when any check or script failed. The results reach the runner through a list shared with the
`Share` API under `kombine.tests`: the report helpers of the examples load
`examples/mkb.results.csx` and append every check to it, child scripts included, so the runner
needs neither the log nor the message handler, which some examples test.
