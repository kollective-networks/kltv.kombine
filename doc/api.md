...
| `bool SetExecutable(KValue Filename, bool executable = true)` | Sets (or clears, with `false`) the executable bits of a file for the user, the group and the others: what a downloaded or copied tool needs to run on Linux and macOS. Nothing to do on Windows, where it returns true. `NotFound` for a missing file. |
...
| `bool Decompress(string tarPath, string outputFolder, bool overwrite = true, bool? showprogress = null)` | Decompresses a tar file into a folder, created if needed. The format is auto-detected (`.tar`, `.tar.gz`, `.tar.bz2`, `.tar.lz`, `.tar.xz`). Outside Windows the permissions of every file are restored from the archive (the executable bit of a tool). Symbolic links are created as links; on Windows, where a link needs a privilege (developer mode or elevation), as a copy of their target when the process may not create them. Hard links are extracted as copies. A link whose target leaves the destination folder is refused like any traversal entry. Entries that would escape the destination folder are refused and, with `overwrite` disabled, existing files are skipped; the rest are still extracted. The call returns false with `AlreadyExists` when only existing files were skipped, `Failed` when entries were refused or not extracted, saying how many. |
...
| `bool DownloadFile(string uri, string path, Dictionary<string,string>? headers = null, bool? showprogress = null, bool executable = false)` | Downloads a file. The destination folder is created if needed; redirects are followed. The progress line reads `Downloading <file>: <progress> done`, or `failed`; `showprogress` overrides `ShowProgress` for this call. Returns `false` on any network or HTTP error (the status is stored in `LastReturnCode`) without aborting the script, and no partial file is left behind — so the script can retry another URL or report its own message. `executable` makes the downloaded file executable once complete (`Files.SetExecutable`, nothing to do on Windows), for a tool downloaded on Linux or macOS. |
| `bool DownloadFiles(string[] uris, string[] paths, Dictionary<string,string>? headers = null, bool? showprogress = null, bool executable = false)` | Downloads multiple files in parallel with one progress line for the batch (`Downloading N files: ...`). Both arrays must have the same length. Returns `false` if any download failed. `executable` makes every downloaded file executable once complete. |
...
| `void CancelCommands()` | Cancels the batch `ExecuteCommands` is running: the queued commands not started yet are not launched and the running ones are killed. May be called from a completion callback (a failed command that must stop the build) or from another thread; `ExecuteCommands` then returns its `Failed` result. Does nothing outside a batch. |
