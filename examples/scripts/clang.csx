...
public class Clang {
	...
	public ToolResult Compile(KList src, KList obj, bool abortwhenfailed = true, bool rebuild = false) {
		...
		foreach (KValue v in IncludeDirs)
			includes += "-I" + v + " ";
		foreach (KValue v in Defines)
			defines += "-D" + v + " ";
		// Sanitize
		...
		switchesCC = SwitchesCC.Flatten().ReduceWhitespace();
		switchesCXX = SwitchesCXX.Flatten().ReduceWhitespace();
		// Create and configure the tool
		Tool tool = new Tool("clang");
		// Set the number of concurrent commands to be executed
		tool.ConcurrentCommands = (uint)ConcurrentBuild;
		...
		for (int a = 0; a != src.Count(); a++) {
			...
			if (src[a].HasExtension(CExtension)){
				cmd = CC;
				args = "-c -MMD "+includes+" "+defines+" "+switchesCC+" "+ src[a] + " -o " + obj[a];
			}else if (src[a].HasExtension(CppExtension)){
				cmd = CXX;
	...
	public ToolResult Linker(KList objs,KValue output,bool SharedLibrary = false, bool abortwhenfailed = true) {
		...
		foreach (KValue v in LibraryDirs)
			libdirs += "-L" + v + " ";
		foreach (KValue v in Libraries)
			libs += "-l" + v + " ";
