...
- The cache goes by content: a script or a module is compiled again when its content or the content of a file it loads changed, a touched or moved file compiles nothing, an edited file compiles only what loads it. See doc/execution.md, The cache
...
- Bin2obj: ELF objects, so a Linux host links with any linker; `Format` selects COFF, ELF or Mach-O whatever the host and `Machine` defaults to the architecture of the host instead of x64. See doc/extensions/bin2obj.md
